-- Robust RPC to fetch LUG list for panels.

create or replace function public.get_lugs_panel_data(p_user_id uuid)
returns table (
  id uuid,
  owner_id uuid,
  name text,
  ...
  select
    ...
    case
      when l.id = me.current_lug_id then
        case
          when l.owner_id = p_user_id then 'admin'
          when exists (
    ...
    case
      when l.id = me.current_lug_id then
        coalesce(
          ...
          case when l.owner_id = p_user_id then 'active' else 'none' end
        )
      else 'none'
...
grant execute on function public.get_lugs_panel_data(uuid) to authenticated;
