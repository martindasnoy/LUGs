drop function if exists public.get_minifig_user_stats_current(uuid);
...
missing_by_set as (
  select
    set_num,
    bool_or(missing_qty > 0) as has_missing,
    coalesce(sum(missing_qty), 0)::integer as missing_pieces
  from missing_parts
  group by set_num
),
stats as (
  select
    ...
    (
      ...
      from missing_by_set
      where has_missing is true
    ) as missing_count,
    (
      select coalesce(sum(missing_pieces), 0)::integer
      from missing_by_set
      where has_missing is true
    ) as missing_pieces_count,
    (
      select count(*)::integer
